...
3. **`src/hooks/useLogbook.tsx`** — `LogbookProvider`, the app-wide store. Every mutation funnels through `syncAfter` (the single notification-rebuild epilogue — it re-reads truth from the db, never render-scope state). The theme preference (`Appearance.setColorScheme`) is applied here too. The dev-only sample-data seeder lives in `src/dev/sampleData.ts`, wired from the Data sub-screen under `__DEV__` — release builds never offer it; strip the module (and the two buttons) before shipping.

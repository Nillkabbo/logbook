...
3. **`src/hooks/useLogbook.tsx`** — `LogbookProvider`, the app-wide store. Every mutation funnels through `syncAfter` (the single notification-rebuild epilogue — it re-reads truth from the db, never render-scope state). Contains a dev-only sample-data seeder (2 months or 1 year, deterministic PRNG; its Data sub-screen buttons render only under `__DEV__`, so release builds never offer it — strip the seeder itself before shipping). The theme preference (`Appearance.setColorScheme`) is applied here too.
